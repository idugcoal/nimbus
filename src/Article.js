...
export default class Page extends React.Component {
  ...
  fetchPage(props) {
    if (props.prismicCtx) {
      ...
      return props.prismicCtx.api.getByUID('article', props.match.params.uid, {}, (err, doc) => {
        if (doc) {
  ...
  if (this.state.doc) {
    return (
      <div data-wio-id={this.state.doc.id}>
        ...
        <p>{this.state.doc.data.date}</p>
        {/* This is how to insert a Rich Text field into your template as html */}
        {PrismicReact.RichText.render(this.state.doc.data.body)}
      </div>
    );
